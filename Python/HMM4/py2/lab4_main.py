...
def data(file):
    ...
    for i in xrange(len(raw_list)):
        for j in xrange(len(raw_list[i])):
            if j == 0 or j == 1:
...
def BaumWelch(A,B,pi,O):
    ...
    while True:
        ...
        c0 = 0
        a0 = [0 for i in xrange(len(pi[0]))]
        for i in xrange(len(pi[0])):
            a0[i] = pi[0][i]*B[i][O[0]]
            c0 = c0 + a0[i]
        ...
        # Compute a[t][i]
        a = [[0 for i in xrange(len(pi[0]))] for j in xrange(len(O))]
        a[0] = a0
        c = [0 for i in xrange(len(O))]
        c[0] = c0
        #print(c[0])
        ...
        b = [[0 for i in xrange(len(pi[0]))] for j in xrange(len(O))]
        for i in xrange(len(pi[0])):
            b[-1][i] = c[-1]

        # b-pass
        for t in reversed(xrange(len(O)-1)):
            for i in xrange(len(pi[0])):
                b[t][i] = 0
                for j in xrange(len(pi[0])):
        ...
        # 4. Compute g
        g = [[[0 for i in xrange(len(pi[0]))] for j in xrange(len(pi[0]))]
        ...
        # Re-estimate B
        for i in xrange(len(pi[0])):
            for j in xrange(len(getRow(B,0))):
                numer = 0
                denom = 0
                for t in xrange(len(O)):
                    if O[t] == j:
