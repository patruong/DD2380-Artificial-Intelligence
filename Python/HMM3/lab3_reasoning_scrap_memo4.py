...
def Viterbi(At, Bt, Pt, Ot):
    ...
    for k in range(len(Ot)): #Use len(Ot)
        if k == 0:
            ...
            #sequence.append(elemOfMaxProb[d_list.index(max(d_list))])
    #print(d_list.index(max(d_list)))
    #print(maxStates)

    lastState = d_list.index(max(d_list))
    seq = [lastState]
    for i in reversed(range(len(maxStates))):
        #print(maxStates[i][lastState])
        seq.append(maxStates[i][lastState])
        lastState = maxStates[i][lastState]
    #print(seq)
    return list(reversed(seq))
        
    #lastState = At[sequence[-1]].index(max(At[sequence[-1]]))
    #sequence.append(lastState)
    #return sequence
