...
int main(){
    ...
    */
    ...
    // Fill B_temp
    double **B_temp = new double *[p];
    for (i = 0; i < p; ++i){
        B_temp[i] = new double[q];
    } 
    for (i = 0; i < p; ++i){
        for (j = 0; j < q; ++j){
            B_temp[i][j] = B[i][j];
    ...
    for (i = 0; i < p; ++i){
        for(j = 0; j < q; ++j){
            if( fabs(B[i][j] - B_temp[i][j]) < abs(0.2)){
                B_temp[i][j] = B[i][j];
            }
            else{}
        }
    ...
    if (A == A_temp){
        cout << "A converged at: " << iters << endl;
    }
    if (B == B_temp){
        cout << "B converged at: " << iters << endl;
    }

    /**************************************/
    cout << logProb << " "  << oldLogProb << endl;
     
    if ((logProb - oldLogProb) < 0.000000001){
