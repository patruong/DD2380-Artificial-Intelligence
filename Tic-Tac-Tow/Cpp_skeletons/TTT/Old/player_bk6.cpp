...
int minimax_val(const GameState &pState, uint8_t Player, int count){
    ...
    std::cerr << "COUNT NOE " << count << std::endl;
    /*
   ...
   */
    ...
    int sizeVec = lNextStates.size();
    //std::cerr << "sizeVEc "<< sizeVec << std::endl;
    if (sizeVec < 16){
        //std::cerr << "HALLAL" << std::endl;
        std::cerr << "HALLAL" << count << std::endl;
        if (count == 3){
    ...
    //Here we want to return +-1 or 0
        ...
        if (Player == 1){ // CELL_X){ //mNextPlayer is private
            ...
            int bestPossible = std::numeric_limits<int>::min();//-INFINITY;
