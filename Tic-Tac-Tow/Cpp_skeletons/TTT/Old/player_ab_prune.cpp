...
int minimax_val(const GameState &pState, uint8_t Player, int count){
    ...
    if (count ==  2){// && lNextStates.size() > 4){ //4 is possible
        //std::cerr << "Total is " << totalValue << std::endl;
        //std::cerr << totalValue << std::endl;
        return totalValue;
    }
    ...
    //Here we want to return +-1 or 0
        ...
        if (Player == 1){ // CELL_X){ //mNextPlayer is private
            ...
            //std::cerr << "BestPossible is " << bestPossible << std::endl;
            return bestPossible;
        }
        else{
            int size = lNextStates.size();
...
///////////ALPHA-BETA-PRUNE///////////////////////////
int alphabeta(const GameState &pState, int depth, int a, int b, uint8_t Player){
    ...
    totalValue = totalValue + value_d1[0] + value_d2[0] + value_spec[0] + value_spec2[0]; 
    */
    //a = current best value of value achievable by A
    //b = current best value of vlaue achievable by B
    if (depth == 0 || lNextStates.size() == 0){
        //std::cerr << "FOOOCK " << totalValue << std::endl;
        //int v = totalValue;
        if (pState.isXWin() == true){
            //std::cerr << "X win return" << std::endl;
            return 10000;}
        ...
        if (Player == 2){
            ...
            for (int i = 0; i < lNextStates.size(); i++){
                v = std::max(v, alphabeta(lNextStates[i], depth-1, a, b, pState.getNextPlayer()));
                a = std::max(a, v);
                //std::cerr << "v " << v << "a " << a << std::endl;
                if (b <= a){
                    return v;
                    break;
                }
            ...
            int v = 100000;
            for (int i = 0; i < lNextStates.size(); i++){
                v = std::min(v, alphabeta(lNextStates[i], depth-1, a, b, pState.getNextPlayer()));
                b = std::min(b, v);
                if (b <= a){
                    return v;
                    break;
...
GameState Player::play(const GameState &pState,const Deadline &pDue)
{
    ...
    */
    ...
    for (int i = 0; i < lNextStates.size(); ++i){
        ...
        //temp = alphabeta(lNextStates[i], 3, -1000,1000, pState.getNextPlayer());
