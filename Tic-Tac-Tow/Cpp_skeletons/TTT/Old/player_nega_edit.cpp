...
/////////////HEURESTICS FOR Y/////////////////////////////////////
// CHECK row-heurestics
int CheckX_2(const GameState &pState, int value[]){
    int k = 0;
    //int value[4] = {0,0,0,0};
    for(int i = 0; i < 4; i++){
        value[i] = 0;
...
// CHECK col-heurestics
void CheckY_2(const GameState &pState, int value[]){
    int k = 0;
    //int value[4] = {0,0,0,0};
    for(int i = 0; i < 4; i++){
        value[i] = 0;
...
// CHECK special case diagonal 1
void CheckD1_2(const GameState &pState, int value[]){
    int diag[4] = {0,5,10,15};
    value[0] = 0;
    for (int i = 0; i < 4; ++i){
        if (unsigned(pState.at(diag[i])) == 1){
...
// CHECK special case diagonal 2
void CheckD2_2(const GameState &pState, int value[0]){
    int diag[4] = {3,6,9,12};
    value[0] = 0;
    for (int i = 0; i < 4; ++i){
        if (unsigned(pState.at(diag[i])) == 1){
...
//occupying 3 corners give bonus
void CheckSC_2(const GameState &pState, int value[0]){
    int corners[4] = {0,3,12,15};
    value[0] = 0;
    if (pState.at(0) == 1 && pState.at(3) == 2 && pState.at(15) == 2){
        value[0] = -100;
    }
    if (pState.at(3) == 1 && pState.at(15) == 2 && pState.at(12) == 2){
        value[0] = -100;
    }
    if (pState.at(15) == 1 && pState.at(12) == 2 && pState.at(0) == 2){
        value[0] = -100;
    }
    if (pState.at(12) == 1 && pState.at(0) == 2 && pState.at(3) == 2){
        value[0] = -100;
    }
}
...
int minimax_val(const GameState &pState, uint8_t Player, int count){
    ...
    if(lNextStates.size() == 0){
        ...
        if (pState.isOWin() == true){
            ...
            return -1000;} // Find how to do this?  
    ...
    int totalValue = 0;  
    // HEURESTICS FUNCTION
    // HEURESTICS IF WE ARE PLAYER 1 - nextPlayer is then 2
    ...
    // HEURESTIC IF WE ARE PLAYER 2 - nextPlayer is 1
    ...
    int value_d2[1];
    int value_spec[1];
    CheckX_2(pState, value_r);
    CheckY_2(pState, value_c);
    CheckD1_2(pState, value_d1);
    CheckD2_2(pState, value_d2);
    CheckSC_2(pState, value_spec);
    ...
    if (count == 3 && lNextStates.size() > 5){ //4 is possible
