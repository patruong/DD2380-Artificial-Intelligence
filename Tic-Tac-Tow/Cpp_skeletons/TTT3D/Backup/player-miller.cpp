...
int evalFunc(const GameState &pState, uint8_t Player){
    ...
    //};
    if (pState.at(0) == 1){util+=5;}
    if (pState.at(3) == 1){util+=5;}
    if (pState.at(12) == 1){util+=5;}
    if (pState.at(15) == 1){util+=5;}
    if (pState.at(48) == 1){util+=5;}
    if (pState.at(51) == 1){util+=5;}
    if (pState.at(60) == 1){util+=5;}
    if (pState.at(63) == 1){util+=5;}

    if (pState.at(0) == 2){util-=5;}
    if (pState.at(3) == 2){util-=5;}
    if (pState.at(12) == 2){util-=5;}
    if (pState.at(15) == 2){util-=5;}
    if (pState.at(48) == 2){util-=5;}
    if (pState.at(51) == 2){util-=5;}
    if (pState.at(60) == 2){util-=5;}
    if (pState.at(63) == 2){util-=5;}
...
GameState Player::play(const GameState &pState,const Deadline &pDue)
{
    ...
    */
    ...
    for (unsigned i = 0; i < lNextStates.size(); ++i){
        //TODO
        temp = std::numeric_limits<int>::max(temp,minimax_val(lNextStates[i], pState.getNextPlayer(), 0, maxDepth,  std::numeric_limits<int>::min(),  std::numeric_limits<int>::max()));
        if(pState.getNextPlayer() == 2){
            if(temp > maxVal){
                maxVal = temp;
                choice = i;
