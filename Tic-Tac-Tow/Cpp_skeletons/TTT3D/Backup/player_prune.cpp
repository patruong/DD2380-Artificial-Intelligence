...
GameState Player::play(const GameState &pState,const Deadline &pDue)
{
    ...
    */
    ...
    if(pState.getNextPlayer() == 2){
            ...
            v = std::numeric_limits<int>::min();
            for(int i = 0; i < size; ++i){
                v = std::max(v, minimax_val(lNextStates[i], pState.getNextPlayer(), 0, maxDepth, alpha, beta));
                alpha = std::max(v, alpha);
                if (beta < alpha || beta == alpha){
                    count = i;
                    break;
            ...
            v = std::numeric_limits<int>::max();
            for(int i = 0; i < size; ++i){
                v = std::min(v, minimax_val(lNextStates[i], pState.getNextPlayer(), 0, maxDepth, alpha, beta));
                beta = std::min(v, beta);
                if (beta < alpha || beta == alpha){
                    count = i;
                    break;
