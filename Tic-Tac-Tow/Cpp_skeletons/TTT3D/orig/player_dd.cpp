...
GameState Player::play(const GameState &pState,const Deadline &pDue)
{
    ...
    /*
    int heurestics[lNextStates.size()];
    for(int i = 0; i < lNextStates.size(); ++i){
        heurestics = evalFunc(lNextStates[i]);
    }
    
    for(int i = 0; i < lNextStates.size(); ++i){
        heurestics[i] = heurestics[i]*(1 << 2*lNextStates.size()) + i;
    }
    std::vector<int> myvector(heurestics, myints+(lNextStates.size());
            sort(myvector.begin(), myvector.begin()+(lNextStates.size()), std:less<int>());*/
